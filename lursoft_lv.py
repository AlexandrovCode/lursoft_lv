...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        self.get_working_tree_api(f'https://www.lursoft.lv/meklet?q={searchquery.replace(" ", "+")}', 'tree')

        result = self.get_by_xpath('//table[@class="lurs-search lurs-search--responsive lurs-search--body"]//tr/td[3]//a/@href')
    ...
    def fill_business_classifier(self, xpathCodes=None, xpathDesc=None, xpathLabels=None, api=False):
        ...
        try:
            text = self.get_by_xpath(xpathCodes) if not api else [self.get_by_api(xpathCodes)]
            #print(text)
            text = ' '.join(text[1:-1])
    ...
    def get_post_address(self, xpath=None, zipPattern=None, key=None, returnAddress=False, addr=None):
        if xpath:
    ...
    def fillField(self, fieldName, key=None, xpath=None, test=False, reformatDate=None):
        ...
        if el:
            ...
            if fieldName == 'isIncorporatedIn':
                if reformatDate:
                    self.overview[fieldName] = self.reformat_date(el.split(', ')[-1], reformatDate)
                else:
    ...
    def fill_identifiers(self, xpathTradeRegistry=None,
    ...
    def get_overview(self, link_name):
        ...
        #self.check_tree()
        try:
            self.fillField('vcard:organization-name', xpath='//td[@class="infotag responsive_hide"]/text()[contains(., "Name")]/../following-sibling::td[1]//span[2]/text()')
        except:
            return None
        ...
        self.fill_business_classifier(xpathCodes='//td[@class="infotag responsive_hide"]/text()[contains(., "Activity code (NACE)")]/../following-sibling::td[1]//text()')


        self.get_address(xpath='//td[@class="infotag responsive_hide"]/text()[contains(., "Legal address")]/../following-sibling::td[1]//span/a/span/text()',
        ...
        self.fillField('isIncorporatedIn', xpath='//td[@class="infotag responsive_hide"]/text()[contains(., "Registration number, date")]/../following-sibling::td[1]//span[2]/text()',
                       reformatDate='%d.%m.%Y')

        self.fillField('tr-org:hasRegisteredPhoneNumber',
                       xpath='//td[@class="infotag"]/text()[contains(., "Phone")]/../following-sibling::td[1]/text()',
                       reformatDate='%d.%m.%Y')
        self.fill_identifiers(xpathOtherCompanyId='//td[@class="infotag responsive_hide"]/text()[contains(., "SEPA identifier")]/../following-sibling::td[1]/text()',
                              xpathVat='//td[@class="info-header"]/text()[contains(., "VAT identification number")]/../../following-sibling::tr[1]/td/text()')

        self.fillField('lei:legalForm', xpath='//td[@class="infotag responsive_hide"]/text()[contains(., "Legal form")]/../following-sibling::td[1]/text()')

        self.fillField('bst:registrationId', xpath='//td[@class="infotag responsive_hide"]/text()[contains(., "Registration number, date")]/../following-sibling::td[1]//span[2]/text()')

        self.fillField('map', xpath='//img[@class="visible-print w100p"]/@src')



        self.get_working_tree_api(f'https://www.lursoft.lv/zo_get.php?code={link_name.split("/")[-1][1:-1]}&language=en', 'tree')
        #self.check_tree()

        #print(self.get_by_xpath('//div[@class="vizitka_contact_web"]/a/@href'))
        self.fillField('hasURL',
                       xpath='//div[@class="vizitka_contact_web"]/a/@href')

        try:
            mail = self.get_by_xpath('//div[@class="vizitka_contact_web"]/script/text()')[0]
    ...
    def get_shareholders(self, link_name):
        ...
        try:
            names = self.get_by_xpath('//td[@class="info-header toogle-block"]/text()[contains(., "List of shareholders")]/../../../following-sibling::tbody/tr/td/span/span/strong//text()')
            for name in names:
                ...
                try:
                    totPer = self.tree.xpath(f'//td[@class="info-header toogle-block"]/text()[contains(., "List of shareholders")]/../../../following-sibling::tbody/tr/td/span/span/strong/a/text()[contains(., "{name}")]/../../../../../../following-sibling::tr[1]/td[5]/text()')
                    if not totPer:
                ...
                try:
                    addr = self.tree.xpath(f'//td[@class="info-header toogle-block"]/text()[contains(., "List of shareholders")]/../../../following-sibling::tbody/tr/td/span/span/strong/a/text()[contains(., "{name}")]/../../following-sibling::span[2]/text()')
                    if not addr:
                ...
                try:
                    sid = self.get_by_xpath(f'//td[@class="info-header toogle-block"]/text()[contains(., "List of shareholders")]/../../../following-sibling::tbody/tr/td/span/span/strong//text()[contains(., "{name}")]/../../..//text()[1]')
                    sid = ' '.join(sid)
                    sid = re.findall('\(.+\)', sid)
                    if sid:
                        sid = sid[0].split(' ')[-1][:-1]
                ...
                try:
                    incDate = self.tree.xpath(f'//td[@class="info-header toogle-block"]/text()[contains(., "List of shareholders")]/../../../following-sibling::tbody/tr/td/span/span/strong/a/text()[contains(., "{name}")]/../../../../../../following-sibling::tr[1]/td[1]/text()')
                    if not incDate:
                    ...
                    basic_in['incorporationDate '] = self.reformat_date(incDate, '%d.%m.%Y')
